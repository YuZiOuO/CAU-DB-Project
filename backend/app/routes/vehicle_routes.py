...
from backend.app.models.models import VehicleType, Vehicle, Rental
...
def get_vehicle_types():
    ...
    return jsonify({
        'code': 200,
        'msg': 'Success',
        'data': [vt.to_dict() for vt in vehicle_types]
    })
...
def create_vehicle_type():
    ...
    if not user or not user.is_admin:
        ...
        }), 200  # Return 200 as per spec, but with error code in JSON
...
def delete_vehicle_type(type_id):
    ...
    if vehicle_type.vehicles:
        return jsonify({
            'code': 400,
            'msg': 'Cannot delete vehicle type with associated vehicles'
        }), 200
...
def create_vehicle():
    ...
    data = request.json
    if not data or not all(k in data for k in ('type_id', 'manufacture_date', 'store_id')):
        return jsonify({
            'code': 400,
...
def update_vehicle(vehicle_id):
    ...
    if 'manufacture_date' in data:
        try:
            vehicle.manufacture_date = datetime.strptime(data['manufacture_date'], '%Y-%m-%d').date()
        except ValueError:
            return jsonify({
                'code': 400,
                'msg': 'Invalid date format. Use YYYY-MM-DD'
    ...
    if 'store_id' in data:
        ...
        active_rental = Rental.query.filter_by(vehicle_id=vehicle_id, rental_status='active').first()
        if active_rental:
            return jsonify({
                'code': 400,
        ...
        ).filter(VehicleTransfer.transfer_status.in_(['pending', 'approved'])).first()
        if pending_transfer:
            return jsonify({
                'code': 400,
...
def delete_vehicle(vehicle_id):
    ...
    # Check if vehicle is currently rented
    active_rentals = Rental.query.filter_by(vehicle_id=vehicle_id).filter(Rental.rental_status.in_(['pending', 'active'])).first()
    if active_rentals:
        return jsonify({
            'code': 400,
            'msg': 'Cannot delete vehicle with active rentals'
        }), 200
